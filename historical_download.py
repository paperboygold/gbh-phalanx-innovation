...
target_file_patterns = [
    r".*PUBLIC_DVD_DISPATCHPRICE.*\.zip$",
    r".*PUBLIC_DVD_DISPATCHREGIONSUM.*\.zip$",
    r".*PUBLIC_DVD_PREDISPATCHREGIONSUM.*_D_.*\.zip$",
    r".*PUBLIC_DVD_PREDISPATCHPRICE(?<!SENSITIVITIE)_D_.*\.zip$",
    r".*PUBLIC_DVD_TRADINGPRICE_.*\.zip$"
]

# Base URL for aggregated pricing data
price_demand_base_url = "https://aemo.com.au/aemo/data/nem/priceanddemand/"
...
def generate_urls(start_year, start_month, end_year, end_month):
    ...
    while current_date <= end_date:
        ...
        url = f"{base_url}{year}/MMSDM_{year}_{month}/MMSDM_Historical_Data_SQLLoader/DATA/"
        urls.append(url)
...
def download_price_demand_data(start_year, start_month, end_year, end_month, regions):
    start_date = datetime.date(start_year, start_month, 1)
    end_date = datetime.date(end_year, end_month, 1)
    current_date = start_date
    ...
    while current_date <= end_date:
        ...
        for region in regions:
            file_name = f"PRICE_AND_DEMAND_{year_month}_{region}.csv"
            url = f"{price_demand_base_url}{file_name}"
            links.append(url)
        if current_date.month == 12:
            current_date = datetime.date(current_date.year + 1, 1, 1)
        else:
    ...
    failed_downloads = download_files(links, data_directory)
    if failed_downloads:
        logging.error(f"Failed to download the following price and demand files: {failed_downloads}")
...
def main():
    ...
    download_price_demand_data(start_year, start_month, end_year, end_month, regions)
